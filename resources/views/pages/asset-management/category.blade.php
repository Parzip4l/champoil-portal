            ...
            <div class="card-header">
                <div class="row">
                    <div class="col-md-6 align-self-center">
                        <h6 class="card-title mb-0">Asset Category Master</h6>
                    </div>
                                ...
                                <td>
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                            ...
                                            <form action="{{ route('asset-category.destroy', $data->id) }}" method="POST" id="delete_contact" class="contactdelete"> 
                                                @csrf @method('DELETE') 
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="ModalAssets">Tambah Kategori Assets</h5>
            ...
            <div class="modal-body">
                <form action="{{ route('asset-category.store') }}" method="post" enctype="multipart/form-data">
                    ...
                    <button type="submit" class="btn btn-primary w-100">Tambahkan Kategori</button>
                </form>
            </div>
        </div>
    </div>
...
<div class="modal fade" id="ModalAssets{{$data->id}}" tabindex="-1" aria-labelledby="ModalAssets" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="ModalAssets">Update Data Kategori</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="btn-close"></button>
            </div>
            <div class="modal-body">
                <form action="{{ route('asset-category.update', $data->id) }}" method="post" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div class="form-group mb-3">
                        <label for="Judul" class="form-label">Name</label>
                    ...
                    <button type="submit" class="btn btn-primary w-100">Update Asset Kategori</button>
                </form>
            </div>
        </div>
    </div>
    ...
    function showDeleteDataDialog(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                const deleteUrl = "{{ route('asset-category.destroy', ':id') }}".replace(':id', id);
                fetch(deleteUrl, {
                    method: 'DELETE',
                    headers: {
