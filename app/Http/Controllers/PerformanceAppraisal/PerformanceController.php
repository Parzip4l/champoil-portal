    ...
    public function downloadPDF()
    {
        ...
        foreach ($employees as $employee) {
            ...
            if (!$paData->isEmpty()) {
                ...
                // Ambil periode dan tahun
                $periode = $paData->first()->periode;
                $tahun = $paData->first()->tahun;
            }

            $tahunReport = $tahun;
            $periodeReport = $periode;
        ...
        $pdf = Pdf::loadView('pages.hc.pa.report', compact('performanceData'))
            ->setPaper('a4', 'landscape'); // Atur ukuran kertas dan orientasi

        return $pdf->download('Performance_Appraisal_Report.pdf');
    }
